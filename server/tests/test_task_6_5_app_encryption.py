...
def test_task_6_5_appenc_plaintext_compat() -> None:
    keys = {"k1": b"a" * 32}
    ...
    with TestClient(app) as client:
        ...
        resp = client.post("/api/v1/saves", json={"name": "plain-ok"}, headers=headers)
        assert resp.status_code == 201, resp.text
        body = cast(dict[str, object], resp.json())
        assert isinstance(body.get("id"), str)


def test_task_6_5_appenc_encrypt_req_and_resp() -> None:
    key = b"b" * 32
    keys = {"k1": key}
    app = create_app(_make_settings(keys))
...
def test_task_6_5_appenc_replay_rejected() -> None:
    key = b"c" * 32
    keys = {"k1": key}
    app = create_app(_make_settings(keys))
    ...
    with TestClient(app) as client:
        ...
        body = json.dumps(env, ensure_ascii=True, separators=(",", ":"))

        r1 = client.post("/api/v1/saves", headers=req_headers, content=body)
        assert r1.status_code == 201, r1.text

        r2 = client.post("/api/v1/saves", headers=req_headers, content=body)
        assert r2.status_code == 409, r2.text
        err = cast(dict[str, object], r2.json())
        assert err.get("detail") == "PARA_APPENC_REPLAY"
