## [2026-02-23 10:23] - 任务：清理 workplans（plans + boulder）
- 原始需求：用户要求清理当前所有 workplans。
- 实现目标/逻辑：
  - 清空 `.sisyphus/plans/`（仅保留目录结构），避免 `/start-work` 继续发现旧计划。
  - 从 git 中移除曾经被误跟踪的 plan 与 `.sisyphus/boulder.json`。
  - 在 `.gitignore` 忽略 `.sisyphus/boulder.json`，避免后续状态文件被再次提交。
- 核心变更：`.sisyphus/plans/ai-girlfriend-desktop-pet.md`、`.sisyphus/boulder.json`、`.gitignore`。

## [2026-02-23 08:16] - 任务：恢复工作会话（client-ux-auth-i18n-pet）
- 原始需求：执行 `/start-work`，恢复 `.sisyphus` 的 active plan 状态并继续未完成计划。
- 实现目标/逻辑：
  - 更新 `.sisyphus/boulder.json` 追加当前 session_id。
  - 同步 `.sisyphus/plans/client-ux-auth-i18n-pet.md` 勾选已完成项，避免重复劳动。
  - 后续剩余项聚焦 Release 与 Final Verification（涉及 push/tag 必须用户显式确认）。
- 核心变更：`.sisyphus/boulder.json`、`.sisyphus/plans/client-ux-auth-i18n-pet.md`。
- 遗留/约束：不做任何 push/tag；发布动作需用户确认。

## [2026-02-23 08:25] - 任务：Final Verification（F1/F3）+ Release 预检（不 push/tag）
- 原始需求：按计划补齐 Final Verification：
  - F1：至少验证一次完整注册 -> 退出登录 -> 使用 username 登录 -> 进入 Chat。
  - F3：至少在 Linux 验证桌宠可交互/拖拽/位置记忆；并给出 Windows 复现步骤。
  - Release：仅做发布前本地一致性检查，不做任何 push/tag。
- 实现目标/逻辑：
  - 新增 Playwright Electron 用例 `electron-auth-happy.spec.ts`，stub server 支持 username->email 映射，覆盖完整 auth happy path。
  - Linux 侧跑 `electron-smoke` + `electron-pet-ui`；Windows 侧输出命令与手工兜底步骤。
  - 运行 `python scripts/sync_versions.py --check` 作为 release 预检。
- 核心变更：`client/playwright/electron-auth-happy.spec.ts`。
- 证据：
  - `.sisyphus/evidence/final-f1-auth-happy.txt`
  - `.sisyphus/evidence/final-f1-auth-happy.png`
  - `.sisyphus/evidence/final-f3-linux-smoke.txt`
  - `.sisyphus/evidence/final-f3-windows-steps.txt`
  - `.sisyphus/evidence/final-release-prep-sync-versions.txt`
- 遗留/约束：如需发布，必须在用户明确允许后执行 commit/push/tag。

## [2026-02-22 19:23] - 任务：Task 17（client：Electron Playwright 覆盖登录/注册入口、中文导航、桌宠交互与位置记忆）
- 原始需求：Electron E2E 覆盖侧边栏“登录/注册”入口与登录/注册互链；覆盖侧边栏中文导航（聊天/设置/插件/知识库/更新）；覆盖桌宠可交互（pet-sprite 打开菜单）+ 拖拽把手移动窗口 + 同一 profile 重启后 bounds 位置记忆；并确保指定 e2e 命令通过。
- 实现目标/逻辑：
  - auth stub server 兼容新版 `{identifier,password}` 与 legacy `{email,password}`，避免客户端升级后 stub 400。
  - 中文导航断言 scope 到 `nav[aria-label=Primary]`，用 role+name 校验少量关键入口。
  - 桌宠窗口优先通过 `window=pet` query 定位；bounds 相关断言使用 `desktopApi.pet.getBounds()` + `expect.poll` 抗抖动。
  - bounds 记忆：移动后先等待 `para.ui.json` 写入生效，再同 profile 重启对比恢复。
- 核心变更：`client/playwright/electron-login.spec.ts`、`client/playwright/electron-register.spec.ts`、`client/playwright/electron-pet-ui.spec.ts`。
- 验证：`cd client && npm run e2e -- electron-smoke.spec.ts`、`cd client && npm run e2e -- electron-login.spec.ts electron-register.spec.ts electron-pet-ui.spec.ts` 通过。

## [2026-02-22 18:51] - 任务：Task 15（桌宠位置记忆：保存/恢复 bounds，并 clamp 到可视区域）
- 原始需求：移动桌宠窗口后把 bounds 持久化到磁盘（约 300ms debounce）；下次启动恢复；若保存的 bounds 因显示器变更跑到屏幕外，则启动时 clamp 回可视 workArea；独立 UI 配置文件（不混入 `para.security.json`），并用 tmp+rename 原子写入且 merge-safe。
- 实现目标/逻辑：
  - 新增 `para.ui.json`（`app.getPath('userData')`）读写：读取校验 `version===1` 且 `petWindowBounds` 为有限数；写入时先读旧 JSON（若可解析）并 merge，再 tmp 写入 + rename。
  - 恢复：创建桌宠窗口时读取保存的 bounds，按 `screen.getAllDisplays()` 的 `workArea` 选择最近显示器并 clamp 到可视区域，在 `ready-to-show` 内先 `setBounds` 再 `showInactive()`，避免跳动。
  - 持久化：桌宠窗口监听 `move/resize`，300ms debounce 落盘；窗口 close 时尝试再落盘一次；closed 时清理 timer/监听。
- 核心变更：`client/src/main/index.ts`。
- 验证：`npm -C client run lint`、`npm -C client run e2e -- electron-smoke.spec.ts` 通过。

## [2026-02-22 17:33] - 任务：Task 14（桌宠 UI：实现可测试的拖拽移动）
- 原始需求：桌宠窗口可通过拖拽一个专用 handle 移动；renderer 用 pointer events 计算 `screenX/screenY` delta，通过 IPC 设置 `BrowserWindow` bounds；提供 `pet:getBounds`/`pet:setBounds`；并通过 `npm -C client run lint`。
- 实现目标/逻辑：
  - main：新增 IPC `pet:getBounds` 返回当前桌宠窗口 bounds；`pet:setBounds` 对 payload 做对象/数值校验，并对 width/height 与 x/y 做钳制（避免异常 payload 导致巨大透明置顶覆盖层或窗口跑到屏幕外）。
  - preload + typings：暴露 `desktopApi.pet.getBounds()` / `desktopApi.pet.setBounds(bounds)`，同步补齐 `client/src/renderer/vite-env.d.ts`。
  - renderer：`client/src/renderer/pet/PetApp.tsx` 增加 `data-testid="pet-drag-handle"`；pointerdown 拉取起始 bounds；pointermove 用 rAF 节流调用 `pet:setBounds`；pointerup/cancel 清理监听，且不吞掉既有 sprite/menu 点击。
- 核心变更：`client/src/main/index.ts`、`client/src/preload/index.ts`、`client/src/renderer/vite-env.d.ts`、`client/src/renderer/pet/PetApp.tsx`。
- 验证：`npm -C client run lint` 通过。

## [2026-02-22 17:03] - 任务：Task 7（客户端：新增正式注册页 /register）
- 原始需求：新增正式注册页 `#/register`（不走 `/dev` 门控）；表单包含用户名（可选）/邮箱/密码/邀请码（可选）；提交后调用 `desktopApi.auth.register(...)` 成功跳转 `#/chat`；错误提示中文映射最小覆盖；并通过 `npm -C client run lint`。
- 实现目标/逻辑：
  - 路由：在 `HashRouter` 增加 `/register`，与 `/login` 同级，确保用户侧可达。
  - 签名选择：以 `username.trim()` 是否为空决定调用旧签名 `register(email,password,invite?)` 或新签名 `register(username,email,password,invite?)`；并通过分支调用避免把 `undefined` 当作显式位置参数传入。
  - 错误映射：沿用 `err.message`/字符串/对象 message 的提取方式，按 `includes(...)` 最小覆盖邀请码相关、`registration_closed`、`username_invalid/username_taken`、邮箱已注册、网络/安全存储错误。
- 核心变更：
  - `client/src/renderer/app/pages/RegisterPage.tsx`：新增正式注册页面。
  - `client/src/renderer/app/RouterApp.tsx`：新增 `/register` 路由并引入页面。
  - `client/src/renderer/app/testIds.ts`：新增 `registerUsername` test id。
- 验证：`npm -C client run lint` 通过。

## [2026-02-22 16:13] - 任务：Task 9（客户端登录：支持“用户名或邮箱”输入）
- 原始需求：登录页输入框改为“用户名或邮箱”；renderer 调用 `desktopApi.auth.login(identifier,password)`；preload/类型改为 `login(identifier,password)`；main IPC handler 接受 `{identifier,password}` 且兼容旧 `{email,password}`，并向后端 `/api/v1/auth/login` 发送 `{identifier,password}`。
- 实现目标/逻辑：
  - UI：登录输入从 `email` 语义调整为 `identifier`（username/email），并同步校验提示与 BAD_CREDENTIALS 文案（不改错误码映射）。
  - IPC：preload 统一发 `{identifier,password}`；main 侧优先用 `payload.identifier`，为空再 fallback 到 `payload.email`，保证历史调用仍可用。
  - 请求体：main 调用后端时统一使用新字段 `{identifier,password}`，即使来源是 legacy `email` 也映射到 `identifier`。
- 核心变更：
  - `client/src/renderer/app/pages/LoginPage.tsx`：文案/placeholder 改为“用户名或邮箱”，调用参数改为 `identifier`。
  - `client/src/preload/index.ts`：`auth.login` 发送 `{identifier,password}`。
  - `client/src/renderer/vite-env.d.ts`：`DesktopApi.auth.login` 参数名对齐为 `identifier`。
  - `client/src/main/index.ts`：`AuthLoginPayload` 与 IPC 校验支持 identifier/email 双形态；login 请求体改为 `{identifier,password}`。
- 验证：`npm -C client run lint` 通过。

## [2026-02-22 16:47] - 任务：Task 12（客户端注册：IPC/类型对齐，auth.register 支持 username）
- 原始需求：`desktopApi.auth.register` 需要支持携带 `username` 注册，并保持旧调用方式仍可用；main 进程 IPC `auth:register` 同时接受旧 payload `{email,password,inviteCode?}` 与新 payload `{username?,email,password,inviteCode?}`；向后端 `/api/v1/auth/register` 发请求时，若 username 非空则包含 `username`；并确保 `npm -C client run lint` 通过。
- 实现目标/逻辑：
  - Preload：`auth.register` 兼容两种签名（旧 `register(email,password,inviteCode?)`，新 `register(username,email,password,inviteCode?)`），并统一转换为对象 payload 发送给 main。
  - 歧义规避：当调用是 3 参数时，通过“第 2 个参数是否包含 `@`”识别新签名；否则按旧签名解析，避免历史 3 参数调用被误判。
  - Main：扩展 `AuthRegisterPayload` 与校验支持可选 `username`；调用后端时仅在 `username.trim()` 非空时写入 `body.username`。
- 核心变更：
  - `client/src/preload/index.ts`：新增 register 新旧签名兼容与 payload 归一化。
  - `client/src/main/index.ts`：`AuthRegisterPayload/isAuthRegisterPayload/registerAndGetMe` 支持可选 `username` 并透传到后端。
  - `client/src/renderer/vite-env.d.ts`：`DesktopApi.auth.register` typings 改为重载签名。
- 遗留/约束：不修改 server 端逻辑/接口；不新增依赖；禁止把任何凭据写入仓库。

## [2026-02-22 16:02] - 任务：Task 13（桌宠窗口：默认可交互，避免“巨大不可交互覆盖层”）
- 原始需求：桌宠窗口启动后默认可交互（不再 mouse-through 覆盖屏幕）；显示时不抢焦点；创建/显示策略降低透明窗闪烁；托盘“切换可交互”映射正确。
- 实现目标/逻辑：
  - 默认值：`petInteractive` 设为 `true`，并在 `createPetWindow()` 创建后按该状态应用 `setIgnoreMouseEvents`。
  - 显示策略：桌宠窗口 `show: false` 创建，在 `ready-to-show` 回调里调用 `showInactive()`（不抢焦点，减少透明窗口闪一下）。
  - 映射：可交互 => `setIgnoreMouseEvents(false)`；穿透 => `setIgnoreMouseEvents(true, { forward: true })`；tray checkbox 使用 `menuItem.checked` 直接设置。
- 核心变更：
  - `client/src/main/index.ts`：`petInteractive` 默认 true；移除无条件 mouse-through；tray show 改 `showInactive()`；统一 interactivity 应用逻辑。
- 验证：`npm -C client run lint` 通过。

## [2026-02-22 15:53] - 任务：后端登录支持 identifier=username|email（兼容旧 email 字段）
- 原始需求：`POST /api/v1/auth/login` 兼容旧 `{email,password}`，同时新增 `{identifier,password}`（identifier 可为 username 或 email）；按输入形态选择查找策略；避免账户枚举；限流 key 使用“有效 identifier”。
- 实现目标/逻辑：
  - 请求体：`LoginRequest` 新增可选 `identifier`，并把 legacy `email` 改为可选；login() 内部优先 `identifier`，两者都缺失/空白则 400 `identifier_required`。
  - 查找策略：包含 `@` -> email（`normalize_email`）；否则 -> username（`strip().lower()`），并据此查询 `User.email` 或 `User.username`。
  - 安全：用户不存在与密码错误统一走 401 + `Bad credentials`。
  - 限流：`auth_rate_limit_key(scope=auth_login, ip, identifier=<有效identifier>)`，成功后 reset，失败 record_failure。
- 核心变更：
  - `server/app/api/v1/auth.py`：扩展 `LoginRequest` + `login()` 逻辑。
  - `server/tests/test_auth.py`：新增用户名登录用例 `test_login_identifier_username_returns_200`。
- 验证：`cd server && uv run pytest -q tests/test_auth.py` -> 8 passed。

## [2026-02-22 14:50] - 任务：后端新增 users.username + Alembic 迁移确定性回填（唯一）
- 原始需求：新增 `users.username` 字段；迁移执行后为既有用户确定性回填且保证唯一；新用户可写入 username；迁移策略必须先加 nullable 列再回填再加唯一约束/索引；禁止在日志中输出回填后的 username/email。
- 实现目标/逻辑：
  - Schema：`users.username` 为 `String(32)` 且 nullable（保持对旧 client/email-only 注册的向后兼容）；ORM 对 username 写入做 `strip().lower()`（空串归一为 NULL）。
  - Backfill：以 email local-part 为 base，lower；非法字符->`_`；压缩 `_`；截断到 32；过短 fallback；冲突按 `_2/_3/...` 追加并裁剪前缀，整体近似 O(n) 且按 `ORDER BY id` 保证确定性。
  - Constraint：回填完成后创建唯一索引 `ux_users_username`。
- 核心变更：
  - `server/app/db/models.py`：User 增加 `username` 字段与写入归一化。
  - `server/alembic/versions/8f35e9f1a5a2_users_username.py`：新增迁移（加列->回填->unique index）。
- 验证：`cd server && uv run alembic upgrade head` 通过。

## [2026-02-22 09:31] - 任务：修复 GitHub Actions 失败（ci/windows/macos/linux）+ Windows 可下载 exe 发布策略确认
- 原始需求：用户反馈 GitHub Actions 失败；需要完整修复并“自动发布一个 Windows exe 可直接下载使用”。
- 实现目标/逻辑：
  - 先把 workflow 本身修到可解析可运行（避免因 expression/context 导致直接 422 / workflow file issue）。
  - 修复 linux-build 的 electron-builder `.deb` 元数据硬性要求（homepage/author.email/maintainer）。
  - 降低 `client_e2e_smoke` 在 Ubuntu runner 的 Electron 启动 flaky（强制 Xvfb + CI 下补充 launch flags，并在 launch 失败时落盘诊断证据）。
- 核心变更：
  - `.github/workflows/ci.yml`：`client_e2e_smoke` 使用 `xvfb-run -a` 包裹 Electron smoke。
  - `.github/workflows/windows-build.yml`：删除“可选签名 enable step”（避免 workflow 校验/上下文问题），改为在打包 step 注入 `CSC_LINK/CSC_KEY_PASSWORD` env。
  - `.github/workflows/macos-build.yml`：把 `CSC_*` 与 `APPLE_*` secrets 统一提升到 job `env:`，并用 `if: ${{ env.CSC_LINK == '' }}` 控制无证书时的 `CSC_IDENTITY_AUTO_DISCOVERY=false`。
  - `client/package.json`：补齐 `homepage`、`author.email`、`build.linux.maintainer` 以满足 `.deb` 构建。
  - `client/playwright/electron-smoke.spec.ts`：CI 下追加 `--no-sandbox/--disable-gpu`，并在 Electron launch 失败时写入 `.sisyphus/evidence/ci-electron-launch-failure.txt`（便于 Actions artifact 排障）。
  - `.github/workflows/nightly-windows-release.yml`：新增 nightly Windows NSIS prerelease（`workflow_dispatch` + daily schedule），自动发布可下载 `.exe` + `.sha256`，tag 采用 `nightly-<YYYYMMDD>-<shortsha>`（避免强制移动固定 tag）。
- 遗留/约束：
  - 以上改动需要 commit + push 后才能在 GitHub Actions 上验证是否全绿。
  - “自动发布 exe”触发策略待用户确认：仅 tag 发布（现有 `release.yml`）或 push main/nightly 自动更新 Release。

## [2026-02-22 01:10] - 任务：F3（Packaged /dev 门控端到端复现 + final-qa 证据）
- 原始需求：在 packaged（linux dir，`client/dist-electron/linux-unpacked/`）场景复现 /dev 门控：未登录时 Settings 打开 desired 仍必须 `effective=false` 且 reason 为 `NOT_LOGGED_IN`（或等价拒绝码）；直接进入 `#/dev/diagnostics` 必须被拒绝并最终重定向到 `#/settings`；产出至少两张脱敏截图到 `.sisyphus/evidence/final-qa/`，并说明 packaged 可执行文件路径与 Playwright 启动方式。
- 实现目标/逻辑：
  - 使用 Playwright Electron 的 `_electron.launch({ executablePath })` 启动 linux-unpacked 主可执行文件；用 `app.windows()` 选取最可能的主窗口（按 `window.innerWidth` 选最大），通过 hash 导航进入 `#/settings` / `#/dev/diagnostics`。
  - 状态隔离：为 packaged 进程注入独立 `HOME/XDG_*` 目录，避免本机登录态或 `para.security.json` 污染导致非确定性。
  - 无图形环境：通过 `xvfb-run -a` 提供 DISPLAY，确保在 headless Linux 上也能拿到窗口并截图。
  - 关键门控断言（fail-closed）：点击 `devOptionsToggle` 后 `devOptionsEffective` 仍为 false；`devOptionsReason` 为 NOT_LOGGED_IN/非 OK；访问 `#/dev/diagnostics` 最终回到 `#/settings`。
- 核心变更：新增 `.sisyphus/scripts/packaged-dev-gating.cjs`（非业务代码，用于 packaged QA 自动化）。
- 证据：
  - `.sisyphus/evidence/final-qa/packaged-settings-dev-options-denied.png`
  - `.sisyphus/evidence/final-qa/packaged-dev-diagnostics-redirected-to-settings.png`
  - `.sisyphus/evidence/final-qa/packaged-dev-gating-summary.json`
- 关键约束/坑：packaged 的 `afterPack-fuses` 会关闭 `EnableNodeCliInspectArguments`，导致 Playwright `_electron.launch` 无法获得 Node inspector 输出而超时；脚本内为自动化**临时** flip 该 fuse 为 true（结束后恢复），不改业务代码。

## [2026-02-22 00:13] - 任务：Task 12（Electron Playwright：/dev 守卫测试从 PARA_DEV_MODE 迁移到 Settings 开关）
- 原始需求：E2E 不再依赖 `PARA_DEV_MODE`；改为在 `#/settings` 显式打开“开发者选项（/dev）”开关后才允许访问 `#/dev/*`。覆盖 deny->allow：开关关闭直达 `#/dev/diagnostics` 必须重定向 `#/settings`；开启后再进 `#/dev/diagnostics` 必须可见 `data-testid=devDiagnostics`；`electron-register` 若访问 `#/dev/register` 必须先开开关；并要求 `cd client && npm run e2e` 全绿、产出 `.sisyphus/evidence/task-12-*.png`。
- 实现目标/逻辑：
  - 开关流程统一走 Settings UI：用 `data-testid=devOptionsToggle` 点击，并等待 `aria-pressed` 与 `data-testid=devOptionsEffective`（解析 `effectiveEnabled`）稳定为 true，避免依赖文案符号（兼容 `：`/`=`/`:`）。
  - deny 场景显式先把开关置为关闭态，再尝试进入 `#/dev/diagnostics`，保证默认值变动时用例仍稳定。
  - Electron 窗口等待用 `expect.poll(() => app.windows().length)`（不使用固定 sleep），并允许某些环境只创建 1 个窗口时降级继续。
- 核心变更：`client/playwright/electron-devmode-guard.spec.ts`、`client/playwright/electron-register.spec.ts`。
- 额外修复（仅为保证全量 e2e 绿）：`client/playwright/electron-plugin.spec.ts` 在安装完成后额外点击一次 Refresh，避免“status 已更新但页面未刷新导致菜单项计数为 0”的 flaky。
- 验证：`cd client && npm run e2e` -> 32 passed, 1 skipped（optional prod spec）。
- 证据：`.sisyphus/evidence/task-12-electron-devoptions-guard-deny.png`、`.sisyphus/evidence/task-12-electron-devoptions-guard-allow.png`。

## [2026-02-22 00:16] - 任务：Electron Playwright：更新 /dev 守卫测试（从 PARA_DEV_MODE 改为 Settings 开关流程）
- 原始需求：更新 Electron Playwright E2E：不再设置/断言 `PARA_DEV_MODE`；改为进入 `#/settings` 点击 `data-testid=devOptionsToggle` 并等待 `devOptionsEffective` 生效，再访问 `#/dev/diagnostics`（allow）或在未开启时重定向到 `#/settings`（deny）；需要为每个场景使用独立临时 userDataDir；运行 `cd client && npm run e2e` 全绿。
- 实现目标/逻辑：
  - /dev 门控：E2E 全部走 Settings UI 开关，等待 `effectiveEnabled：true` 后再进入 `#/dev/*`；deny 场景直接访问 `#/dev/diagnostics` 并用 `expect.poll` 断言 hash 重定向到 `#/settings`。
  - 状态隔离：除 `PARA_USER_DATA_DIR` 外，同时为相关用例注入独立 `XDG_CONFIG_HOME`，避免 `para.security.json`（devOptions/appEnc 等）写入本机真实配置并避免并行用例互相污染。
  - Stub 对齐：所有 E2E stub 的 `GET /api/v1/auth/me` 响应补齐 `debug_allowed: boolean`，匹配 main 侧 `isAuthMeResponse()` 严格校验。
  - 稳定性：补齐必要的 hash 导航等待与 plugins 安装后状态/DOM 刷新等待，降低并行运行时 flaky。
- 核心变更：
  - `client/playwright/electron-devmode-guard.spec.ts`
  - `client/playwright/electron-register.spec.ts`
  - 相关配套修复（为保证 `npm -C client run e2e` 全绿）：`client/playwright/electron-login.spec.ts`、`client/playwright/electron-appenc.spec.ts`、`client/playwright/electron-gallery.spec.ts`、`client/playwright/electron-timeline.spec.ts`、`client/playwright/electron-version.spec.ts`、`client/playwright/electron-userdata-migration.spec.ts`、`client/playwright/electron-theme-focus.spec.ts`、`client/playwright/electron-plugin.spec.ts`、`client/playwright/electron-admin-flag.spec.ts`。
- 证据：`.sisyphus/evidence/task-12-electron-devoptions-guard-deny.png`、`.sisyphus/evidence/task-12-electron-devoptions-guard-allow.png`。
- 验证：`cd client && npm run e2e` -> 32 passed, 1 skipped。

## [2026-02-21 23:41] - 任务：Task 11（admin-web：新增 Debug Users 页面管理 debug_allowed）
- 原始需求：在 `admin-web` 新增“Debug Users”页面：按 email 查询当前 `debug_allowed`；`super_admin` 可切换并保存（PUT）；`operator` 可查看但不可编辑（控件禁用并提示）。API 调用必须走 `apiRequestJson`（admin auth）且不泄露 token；路由挂到 `config/`；`npm -C admin-web run build` 通过。
- 实现目标/逻辑：
  - API：在 `admin-web/src/lib/api.ts` 追加 `adminUsersDebugAllowedGet/adminUsersDebugAllowedPut`，对接 `/api/v1/admin/users/debug_allowed` 的 GET/PUT；响应使用最小类型收敛与兜底，错误提示仅展示 `ApiError.message`（脱敏）。
  - UI：新增 `DebugUsersPage`：email 输入 + 查询按钮；展示当前 `debug_allowed`，使用与 Feature Flags 一致的 `switch` 样式；RBAC 下禁用写操作并提示 Requires super_admin；查询/保存时清理 error/success 状态。
  - 路由：在 `admin-web/src/lib/routes.tsx` 增加 `/config/debug-users` 路由与侧边栏入口（位于 config 分组）。
- 核心变更：`admin-web/src/lib/api.ts`、`admin-web/src/lib/routes.tsx`、`admin-web/src/pages/DebugUsersPage.tsx`。
- 遗留/约束：不改动 server；不引入依赖；避免在错误信息/日志中泄露 token。
- 验证：`npm -C admin-web run build` 通过。

## [2026-02-21 23:22] - 任务：Electron：更新 /dev/diagnostics 页面提示（不再提 PARA_DEV_MODE）
- 原始需求：`/dev/diagnostics` 页面文案改为提示需要在 Settings 开启“开发者选项（/dev）”；正式包需要登录且后端授权 `debug_allowed`；页面不再出现字符串 `PARA_DEV_MODE`。
- 实现目标/逻辑：仅调整页面提示文案（保持改动最小），不修改路由守卫或 Settings UI。
- 核心变更：`client/src/renderer/app/pages/DevDiagnosticsPage.tsx`。
- 验证：`npm -C client run lint`。

## [2026-02-21 23:17] - 任务：Electron：RouterApp /dev 守卫改为 async + fail-closed（不再依赖 PARA_DEV_MODE）
- 原始需求：`#/dev/*` 路由守卫不再使用 `desktopApi.devModeEnabled` 或 `PARA_DEV_MODE`，改为异步调用 `desktopApi.security.getDevOptionsStatus()`；接口不可用/异常时必须 fail-closed 重定向到 `/settings`，并避免 dev 内容首屏闪现。
- 实现目标/逻辑：
  - 显式状态机：`checking | allowed | denied`；初始 `checking` 时渲染 `null`，避免 `<Outlet/>` 闪现。
  - feature-detect：若 `security.getDevOptionsStatus` 缺失，直接 `denied`。
  - 判定规则：仅当返回 `effectiveEnabled === true` 才进入 `allowed`；其它情况（false/异常）统一 `denied`。
  - 稳定性：用 `cancelled` 标志避免组件卸载后 setState。
- 核心变更：`client/src/renderer/app/RouterApp.tsx`。
- 验证：`npm -C client run lint`。

## [2026-02-21 23:40] - 任务：Electron：AuthMe / 类型守卫对齐 debug_allowed（packaged 授权门控）
- 原始需求：桌面端 `/api/v1/auth/me` 已新增返回 `debug_allowed: boolean`，需要在 main/preload/renderer 的类型与 `isAuthMeResponse` 类型守卫里对齐该字段，用于 packaged 下 developer options 的 fail-closed 授权门控。
- 实现目标/逻辑：
  - main：`AuthMeResponse.debug_allowed` 改为必填 `boolean`，`isAuthMeResponse()` 强制要求该字段为 `boolean`（缺失/类型不对即视为 schema 不合法）。
  - preload + renderer typings：`desktopApi.auth.{me,login,register}` 的返回类型补齐 `debug_allowed`，避免 renderer 侧类型漂移。
  - 安全性：`/auth/me` 响应不符合 schema 时继续走现有错误路径（清 token/返回稳定错误码），保证 fail-closed 且不泄露 token。
- 核心变更：`client/src/main/index.ts`、`client/src/preload/index.ts`、`client/src/renderer/vite-env.d.ts`。
- 验证：`npm -C client run lint`。

## [2026-02-20 21:00] - 任务：Final Verification F4（Light 主题可读性 + 主题一致性：renderer 样式去 hardcode）
- 原始需求：修复 Light 主题低对比/偏暗偏绿的“主题漂移”，移除 renderer 组件样式中的 hardcoded dark/green 颜色值，统一改为 tokens（`--text/--muted/--line/--card/--card2/--accent/--accent2/--focus-ring`）驱动；保持 Dark 主题观感基本不变。
- 实现目标/逻辑：
  - `components.css`：标题/按钮/输入框/胶囊/开关/Toast 等组件样式全面去 hardcode，改为 token + `color-mix(...)` 生成的轻/暗自适应颜色。
  - 可用性：为 `ui-toggle` 增加“隐藏 input 聚焦时 track 出现 focus ring”，避免 focus-visible 仅作用在不可见元素上。
  - `base.css`：背景 radial gradient 的主色由 hardcoded green/orange 改为 `--accent/--accent2`，让 light 下背景与强调色一致。
- 核心变更：`client/src/renderer/styles/components.css`、`client/src/renderer/styles/base.css`。
- 验证：待运行 `npm -C client run e2e -- electron-ui-snapshots.spec.ts` 并检查 `.sisyphus/evidence/final-ui/` 的 light 主题截图。

## [2026-02-21 22:08] - 任务：Server /api/v1/auth/me 返回 debug_allowed + pytest
- 原始需求：扩展 `GET /api/v1/auth/me` 响应，新增 `debug_allowed: boolean`（来源 `User.debug_allowed`），并补齐 pytest 覆盖（401/默认 false/true）。
- 实现目标/逻辑：
  - `MeResponse` 增加 `debug_allowed: bool` 字段；`me()` 直接从 ORM 的 `User.debug_allowed` 读取并 fail-safe 转为 `bool(...)`。
  - 测试侧为旧库兜底：`server/tests/conftest.py` 增加 `ALTER TABLE users ADD COLUMN IF NOT EXISTS debug_allowed ...`，避免已存在的测试库缺列导致查询失败。
  - `server/tests/test_auth.py` 新增未登录 401 用例，并断言登录后 `debug_allowed` 默认 `false`；通过 DB 直接把用户标志置 true 后，断言 `/me` 返回 `true`。
- 核心变更：`server/app/api/v1/auth.py`、`server/tests/test_auth.py`、`server/tests/conftest.py`。
- 验证：`cd server && uv run pytest` -> 92 passed。

## [2026-02-21 23:15] - 任务：Server：admin API 管理 users.debug_allowed（仅 super_admin）+ 审计 + pytest
- 原始需求：新增 admin API 管理 `users.debug_allowed`，提供 GET/PUT（仅 super_admin），并在更新时写入 `AuditLog`；补齐 pytest 覆盖（RBAC + 审计 + /api/v1/auth/me 反映）。
- 实现目标/逻辑：
  - 新增 `server/app/api/v1/admin_users.py`：
    - `GET /api/v1/admin/users/debug_allowed?email=...` 返回 `{email, debug_allowed}`
    - `PUT /api/v1/admin/users/debug_allowed` 接收 `{email, debug_allowed}` 并更新 User
  - RBAC：两个端点都依赖 `require_super_admin`，operator 统一 403。
  - Email：使用 `normalize_email()` 统一查询键，避免大小写/空白导致查不到。
  - 审计：PUT 在状态发生变化时写入 `AuditLog`，action 固定为 `user.debug_allowed.update`；metadata_json 仅包含 `{email,user_id,prev,next}`（无敏感信息）。
- 核心变更：`server/app/api/v1/admin_users.py`、`server/app/api/v1/router.py`、`server/tests/test_admin_users_debug_allowed.py`。
- 验证：
  - `cd server && uv run pytest -q -k "debug_allowed and admin"`
  - `cd server && uv run pytest`

## [2026-02-19 23:14] - 任务：Task 6（Dev Mode gating：默认隐藏 + 路由 guard，不可直达）
- 原始需求：当 `PARA_DEV_MODE` 未开启时，访问 `#/dev/*` 必须被重定向离开；当 `PARA_DEV_MODE=1` 时，`#/dev/diagnostics` 可访问且有稳定可断言标识；新增 e2e 用例并产出证据截图。
- 实现目标/逻辑：
  - 单一真值来源：在 preload 暴露 `window.desktopApi.devModeEnabled = (process.env.PARA_DEV_MODE === '1')`，缺省/异常一律视为 false（fail-closed）。
  - 路由 guard：在 `HashRouter` 路由表增加 `/dev/*` 分支；dev mode 关闭时直接 `<Navigate to="/settings" replace />`。
  - Dev 页面：新增 `DevDiagnosticsPage`，提供 `data-testid="devDiagnostics"` 供 Playwright 稳定断言。
  - E2E：新增 `electron-devmode-guard.spec.ts` 覆盖 devMode off/on 两种启动 env，并在 off 场景写入证据图。
- 核心变更：`client/src/preload/index.ts`、`client/src/renderer/vite-env.d.ts`、`client/src/renderer/app/RouterApp.tsx`、`client/src/renderer/app/pages/DevDiagnosticsPage.tsx`、`client/playwright/electron-devmode-guard.spec.ts`。
- 验证：`npm -C client run lint`、`npm -C client test`、`npm -C client run e2e -- electron-smoke.spec.ts`、`npm -C client run e2e -- electron-devmode-guard.spec.ts` 全部通过；证据：`.sisyphus/evidence/task-6-devmode-guard.png`。
- 遗留/约束：保持 `PARA_DEV_MODE` 与 `PARA_LABS` 语义分离；不新增导航入口，避免破坏既有“默认隐藏”合同。

## [2026-02-19 23:44] - 任务：Task 7 FIX（Renderer desktopApi 数据访问层封装 + 登录 e2e 证据）
- 原始需求：Renderer 层封装 `window.desktopApi` 访问，调用点统一走 wrapper（fail-closed）；Electron login 成功路径写出证据图 `.sisyphus/evidence/task-7-login-e2e.png`。
- 实现目标/逻辑：
  - 新增 `client/src/renderer/services/desktopApi.ts`：提供 `getDesktopApi()` / `requireDesktopApi()` / `getUnsubscribe()`，把 renderer 侧的可用性判定与订阅返回值归一化收敛到单点。
  - 替换 renderer 调用点：`client/src/renderer/app/App.tsx`、`client/src/renderer/app/RouterApp.tsx`、`client/src/renderer/pet/PetApp.tsx` 全部移除直接 `window.desktopApi` 访问，改为 `getDesktopApi()` 获取并保持原有错误提示与 fail-closed 分支。
  - E2E：`client/playwright/electron-login.spec.ts` 在 login success 用例中额外写出 `task-7-login-e2e.png`。
- 核心变更：`client/src/renderer/services/desktopApi.ts`、`client/src/renderer/app/App.tsx`、`client/src/renderer/app/RouterApp.tsx`、`client/src/renderer/pet/PetApp.tsx`、`client/playwright/electron-login.spec.ts`。
- 验证：`npm -C client run lint`、`npm -C client test`、`npm -C client run build`、`npm -C client run e2e -- electron-login.spec.ts` 全部通过；证据：`.sisyphus/evidence/task-7-login-e2e.png`。

## [2026-02-19 21:06] - 任务：启动 client UI 重设计与重构（CI 护栏落地）
- 原始需求：用户要求把面向用户发布的桌面客户端 UI “好好重构/重设计”，当前体验像“调试面板”。选择执行计划 `.sisyphus/plans/client-ui-refactor.md`。
- 实现目标/逻辑：
  - 先立住重构护栏：把 client TypeScript typecheck 纳入 CI gate，并新增 Playwright 最小 Electron smoke job，避免 UI 大改期间出现“跑不起来/类型漂移/核心入口崩溃”却无法在 CI 发现。
  - smoke 分层：只跑 `electron-smoke.spec.ts`；`web-admin-*` / `*.prod.*` / `electron-update-rollback` 不进入 PR gate。
- 核心变更：
  - `scripts/ci.sh`：新增 `npm -C client run lint`（tsc typecheck）步骤。
  - `.github/workflows/ci.yml`：新增 `client_e2e_smoke` job（npm ci + playwright install + `npm -C client run e2e -- electron-smoke.spec.ts`）。
  - `.sisyphus/boulder.json`：切换 active_plan 到 `/root/1codes/1para/.sisyphus/plans/client-ui-refactor.md` 并记录 session。
- 遗留/约束：
  - 禁止写入任何真实凭据/Key/内网地址；UI 重构阶段不顺手重写 `client/src/main/index.ts`，只做必要最小增量。
...
## [2026-02-20 14:05] - 任务：Task 11（插件体验产品化：同意/启用/安装 + 保持桌宠联动；E2E 路由化适配）
- 原始需求：两条 Electron plugins 相关 E2E 不再依赖 legacy 默认首页渲染 plugins 卡片；必须显式导航到 `#/plugins` 后再查找 `TEST_IDS.plugins*`，并在导航后用 `TEST_IDS.pluginsCard` 作为同步点。
- 实现目标/逻辑：在 `client/playwright/electron-plugin.spec.ts` 与 `client/playwright/electron-admin-flag.spec.ts` 增加 `navigateToPluginsPage(page)`（`window.location.hash = '#/plugins'` + `expect(...pluginsCard).toBeVisible()`），在 debug panel 步骤开始处先导航再执行 toggle/consent/refresh/select/install 断言，确保用例在当前 HashRouter 架构下稳定通过。
- 核心变更：`client/playwright/electron-plugin.spec.ts`、`client/playwright/electron-admin-flag.spec.ts`、`.sisyphus/evidence/task-11-plugins-e2e.txt`。
- 验证：
  - `npm -C client run e2e -- electron-plugin.spec.ts` -> 1 passed
  - `npm -C client run e2e -- electron-admin-flag.spec.ts` -> 1 passed
...
## [2026-02-20 14:30] - 任务：Task 12（client-ui-refactor：KnowledgePage 路由化 + 投喂闭环 + e2e 对齐）
- 原始需求：提供 `#/knowledge` 页面用于“知识投喂”，保持 `TEST_IDS.feedDropzone/feedProgress/feedDone`；E2E 不再依赖 legacy 首页，需显式 hash 导航；投喂轮询/AbortController 逻辑抽到 service/hook，避免路由切换泄漏。
- 实现目标/逻辑：
  - 新增 `client/src/renderer/app/pages/KnowledgePage.tsx`：拖拽 `.md` 文件后调用 `desktopApi.knowledge.uploadMaterial()`，并通过 `materialStatus()` 轮询直至 `indexed/failed`。
  - 新增 `client/src/renderer/services/knowledgeFeed.ts`：`.md` 判断、错误映射、轮询与 abort（30s timeout，poll 400ms）。
  - 路由接入：`client/src/renderer/app/RouterApp.tsx` 将 `/knowledge` 从 legacy `App` 切换到 `KnowledgePage`。
  - E2E 对齐：`client/playwright/electron-feed-knowledge.spec.ts` 增加 `window.location.hash = '#/knowledge'` 导航，并以 `TEST_IDS.feedDropzone` 可见作为同步点；截图证据写入 `.sisyphus/evidence/task-12-knowledge.png`。
- 验证（本机）：
  - `npm -C client run lint` PASS
  - `npm -C client test` PASS
  - `npm -C client run e2e -- electron-feed-knowledge.spec.ts` -> 1 passed
- 证据：`.sisyphus/evidence/task-12-knowledge.png`

## [2026-02-20 14:50] - 任务：Task 13（client-ui-refactor：BYOK/Vision/Assistant 入口产品化 + e2e 路由化）
- 原始需求：BYOK/视觉/系统助手作为用户可用能力（非 dev-only），入口收纳到 Settings“高级能力”；对应 Electron E2E 在路由化架构下稳定可跑。
- 实现目标/逻辑：
  - Settings 承载高级能力控件：在 `client/src/renderer/app/pages/SettingsPage.tsx` 增加 BYOK/Vision/Assistant 三块 UI，并保持 `TEST_IDS` 合同（`byok*`/`toggleVision`/`visionConsent*`/`toggleAssistant*`/`assistantSuggestion`）。
  - Chat 支持 BYOK 直连：`client/src/renderer/app/pages/ChatPage.tsx` 在 send/stop 路径优先使用 `desktopApi.byok.chatSend/chatAbort`（当 byok enabled=true），否则仍走 WS streaming。
  - E2E 显式 hash 导航：
    - `client/playwright/electron-byok-chat.spec.ts`：先 `#/settings` 配置 BYOK，再 `#/chat` 发送并断言回复；
    - `client/playwright/electron-vision-screenshot.spec.ts`、`client/playwright/electron-assistant-suggest.spec.ts`：先 `#/settings` 再断言与交互。
- 验证（本机）：
  - `npm -C client run lint` PASS
  - `npm -C client test` PASS
  - `npm -C client run e2e -- electron-byok-chat.spec.ts` -> 1 passed
  - `npm -C client run e2e -- electron-vision-screenshot.spec.ts` -> 1 passed
  - `npm -C client run e2e -- electron-assistant-suggest.spec.ts` -> 1 passed
- 证据：`.sisyphus/evidence/task-13-advanced-e2e.txt`

## [2026-02-20 15:05] - 任务：Task 14（client-ui-refactor：PetApp tokens 化 + e2e 对齐）
- 原始需求：PetApp 视觉统一（尽量减少硬编码颜色/阴影），迁移内联 style 到 tokens；保持 `pet-sprite/pet-radial-menu/pet-menu-item-chat/pet-plugin-bubble*` 等 data-testid 合同；`electron-pet-ui` 用例需稳定通过并产出证据。
- 实现目标/逻辑：
  - `client/src/renderer/pet/PetApp.tsx`：保留样式对象结构，但将主要颜色/描边/阴影替换为 tokens（`var(--line)`/`var(--card)`/`var(--card2)`/`var(--text)`/`var(--shadow-card)`），减少主题漂移。
  - `client/playwright/electron-pet-ui.spec.ts`：证据截图文件名对齐到 `.sisyphus/evidence/task-14-pet.png`。
- 验证（本机）：
  - `npm -C client run lint` PASS
  - `npm -C client run e2e -- electron-pet-ui.spec.ts` -> 1 passed
- 证据：`.sisyphus/evidence/task-14-pet.png`

## [2026-02-20 15:15] - 任务：Task 15（client-ui-refactor：默认 UI 去 debug 化 + smoke 合同对齐）
- 原始需求：默认用户体验不应出现 legacy 调试面板；完成从巨石到路由化收口，并保持 `electron-smoke` 合同可跑；同时增加“默认文案关键字扫描”。
- 实现目标/逻辑：
  - 路由收口：`client/src/renderer/app/RouterApp.tsx` 将 `path="/"` 从 legacy `App` 改为重定向到 `/chat`；legacy Debug Panel 迁移到 dev-only 路由 `/dev/debug`（受 `PARA_DEV_MODE=1` 守卫）。
  - smoke 用例对齐：`client/playwright/electron-smoke.spec.ts` 改为显式导航 `#/login` 与 `#/chat` 验证稳定 testids；并在 `#/chat` 上做页面文本扫描，禁止出现关键字：`调试/Task/最小 UI/离线模拟`；扫描结果写入证据文件。
- 验证（本机）：
  - `npm -C client run lint` PASS
  - `npm -C client run e2e -- electron-smoke.spec.ts` -> 1 passed
- 证据：`.sisyphus/evidence/task-15-smoke-and-copy.txt`

## [2026-02-20 15:25] - 任务：Task 16（client-ui-refactor：desktopApi 类型契约收敛：减少 renderer casts）
- 原始需求：收敛 desktopApi 的类型契约，减少页面内 `as any` 与漂移，保持 typecheck 通过。
- 实现目标/逻辑：
  - `client/src/renderer/app/pages/SettingsPage.tsx`：使用 `Window['desktopApi']` 推导类型（例如 `Awaited<ReturnType<...>>`），移除 update/userData/byok/vision 相关 `as any`。
  - `client/src/renderer/app/pages/ChatPage.tsx`：BYOK 启用检测与返回内容使用类型字段（`cfg.enabled`/`resp.content`），减少不必要的 `any`。
- 验证（本机）：`npm -C client run lint` PASS
- 证据：`.sisyphus/evidence/task-16-typecheck.txt`
...
## [2026-02-21 18:28] - 任务：完成 first-user-admin-lockdown 最终验收（收口 + 全量验证）
- 原始需求：继续推进计划 `.sisyphus/plans/first-user-admin-lockdown.md` 的 Final Checklist（5 项未勾选），并确保三类验证命令全绿；修复本机 `web-admin` e2e 失败点。
- 实现目标/逻辑：
  - e2e 稳定性：`client/playwright/web-admin-feature-flags.spec.ts` 的 Electron step 不再依赖 debug panel 默认 hash 路由；显式导航到 `#/plugins` 并以 `TEST_IDS.pluginsCard` 可见作为同步点，然后再做滚动/启用/安装与菜单项断言。
  - 文档收口：将计划文件 Final Checklist 全部勾选，并追加本机可复核的验证命令 PASS 证据；同步更新 `.sisyphus/boulder.json` 的 `updated_at`。
- 核心变更：`client/playwright/web-admin-feature-flags.spec.ts`、`.sisyphus/plans/first-user-admin-lockdown.md`、`.sisyphus/boulder.json`、`.sisyphus/notepads/first-user-admin-lockdown/issues.md`。
- 验证：
  - `cd server && uv run pytest -q` -> PASS
  - `npm --prefix admin-web run build` -> PASS
  - `npm --prefix client run e2e -- --grep web-admin` -> PASS

## [2026-02-20 01:09] - 任务：Task 8（登录/注册页面迁移：路由分流 + e2e 对齐）
- 原始需求（Task 8）：完成登录/注册页面从 legacy 入口迁移到新路由架构，并更新 Playwright 用例以匹配 hash 路由导航与 devMode gating。
- 实现要点：
  - 新增页面：`LoginPage`、`ChatPage`、`DevRegisterPage`。
  - 路由分流：`RouterApp` 按路由将登录、聊天、开发注册等入口拆分并承接导航。
  - legacy 收口：`App.tsx` 的“注册卡片”仅在 devMode 开启时渲染（fail-closed），避免默认 UI 暴露开发入口。
  - E2E 对齐：`electron-login.spec.ts` / `electron-register.spec.ts` 改为通过 hash 导航进入页面，并在需要的用例中概念性启用 `PARA_DEV_MODE=1`（不写入任何真实 env 值到仓库）。
- 验证：`npm -C client run e2e -- electron-login.spec.ts` 与 `npm -C client run e2e -- electron-register.spec.ts` 均通过；证据：`.sisyphus/evidence/task-8-auth-e2e.txt`。

## [2026-02-20 01:43] - 任务：Task 9（Chat 主流程 + streaming batching）
- 原始需求（Task 9）：完成 Chat 主流程并支持 WS 流式输出的 batching，按计划要求补齐 e2e 覆盖与证据留存。
- 实现要点：
  - `ChatPage` 接入 `ws.connect`/`chatSend`/`interrupt`，并通过 `onEvent`/`onStatus` 驱动 UI 状态与消息流转。
  - 增加 token buffer + flush 机制，避免逐 token 渲染造成频繁 re-render，同时保持流式观感。
  - 连接按钮文案固定为“连接”，与用例断言一致。
  - chat stream spec 路由改为 `#/chat`，并将证据文件名统一为 `task-9-chat-stream.png`；新增“流式期间仍可输入”断言，确保输入框在 streaming 时不被锁死。
- 验证：`npm -C client run e2e -- electron-chat-stream.spec.ts` PASS；证据：`.sisyphus/evidence/task-9-chat-stream.png`。

## [2026-02-20 19:41] - 任务：Task 17（A11y 基线：键盘可用 + focus-visible + icon-only aria-label）
- 原始需求：新增/更新 Electron Playwright a11y smoke：覆盖 Login/Chat/Settings/Plugins；验证键盘 Tab 可用、`:focus-visible` 生效；并在发现 icon-only 按钮缺少可访问名时失败；通过后写入固定证据截图。
- 实现目标/逻辑：
  - 路由切换统一用 HashRouter：`window.location.hash = '#/login' | '#/chat' | '#/settings' | '#/plugins'`，并用 `TEST_IDS.*` 作为稳定同步点。
  - focus-visible 直接用 DOM 伪类断言：`document.activeElement.matches(':focus-visible')`（先按 Tab 再断言）。
  - icon-only button baseline：收集可见 `button,[role="button"]` 中 `textContent` 为空且包含 `svg/img` 的元素，要求必须存在 `aria-label`/`title`/`aria-labelledby`。
  - 证据输出：`path.resolve(process.cwd(), '..', '.sisyphus', 'evidence', 'task-17-a11y.png')` 并 `mkdir(..., { recursive: true })`。
- 核心变更：新增 `client/playwright/electron-a11y-smoke.spec.ts`。
- 验证：`npm -C client run e2e -- electron-a11y-smoke.spec.ts` PASS；证据：`.sisyphus/evidence/task-17-a11y.png`。

## [2026-02-20 19:58] - 任务：Task 19（截图回归：Login/Chat/Settings/Plugins 视觉基准输出到 evidence）
- 原始需求：新增/更新 `client/playwright/electron-ui-snapshots.spec.ts`；运行后在 `.sisyphus/evidence/final-ui/` 产出固定文件名的确定性截图（至少 `login/chat/settings/plugins` 的 light 版本，推荐同时输出 dark）；并确保 `npm -C client run e2e -- electron-ui-snapshots.spec.ts` 通过。
- 实现目标/逻辑：
  - Electron 启动方式对齐既有用例：`dist/main/index.js` 存在性断言 + `_electron.launch({ executablePath: electronPath, args: [mainEntry], env: { NODE_ENV: 'test', ... } })`。
  - 窗口选择：复用既有“按 `window.innerWidth` 选最大窗口”为 debug panel page（`getDebugPanelPage`）。
  - 路由与同步点：每个页面都显式设置 `window.location.hash`，并等待对应 `TEST_IDS` 锚点可见后再截图（Login: `loginSubmit`；Chat: `chatInput`；Settings: `userDataCard/updateCard`；Plugins: `pluginsCard`）。
  - 截图确定性：固定 viewport 为 `1280x800`；在 spec 内注入 CSS 关闭 animation/transition 并隐藏 caret，减少焦点/动画造成的像素漂移（不修改业务代码）。
  - 主题确定性：light/dark 两轮分别 `document.documentElement.setAttribute('data-theme', '<theme>')` 后截图。
  - 证据路径：从 e2e cwd=`client/` 使用 `path.resolve(process.cwd(), '..', '.sisyphus', 'evidence', 'final-ui', '<filename>.png')`；`mkdir(..., { recursive: true })` 后 `page.screenshot({ path, fullPage: true })`；再用 `fs.existsSync` 断言落盘。
  - 外网隔离：用本地 http stub server 仅覆盖 `GET /api/v1/feature_flags`（返回 `plugins_enabled=true`），避免 UI 快照依赖真实远端。
- 核心变更：`client/playwright/electron-ui-snapshots.spec.ts`。
- 验证：`npm -C client run e2e -- electron-ui-snapshots.spec.ts` -> 1 passed。
- 证据：
  - `.sisyphus/evidence/final-ui/login-light.png`
  - `.sisyphus/evidence/final-ui/chat-light.png`
  - `.sisyphus/evidence/final-ui/settings-light.png`
  - `.sisyphus/evidence/final-ui/plugins-light.png`
  - `.sisyphus/evidence/final-ui/login-dark.png`
  - `.sisyphus/evidence/final-ui/chat-dark.png`
  - `.sisyphus/evidence/final-ui/settings-dark.png`
  - `.sisyphus/evidence/final-ui/plugins-dark.png`

## [2026-02-20 20:10] - 任务：Task 20（回滚/慢测试分层确认：electron-update-rollback 证据对齐）
- 原始需求：`client/playwright/electron-update-rollback.spec.ts` 需将证据输出改为 `.sisyphus/evidence/task-20-update-rollback.txt`（而非旧的 task-7-2 文件名），并保证 `npm -C client run e2e -- electron-update-rollback.spec.ts` 通过且证据文件落盘。
- 实现目标/逻辑：
  - 证据路径：使用 `path.resolve(process.cwd(), '..', '.sisyphus', 'evidence', 'task-20-update-rollback.txt')`（e2e cwd 为 `client/`），并在写入前 `mkdir(path.dirname(...), { recursive: true })`。
  - 证据自描述：证据内容头部从 `[task-7-2]` 更新为 `[task-20]`，避免任务编号混淆。
  - E2E 稳定性：默认 `#/` 现为重定向到 `#/chat`，因此在用例中显式 `window.location.hash = '#/updates'` 并等待 `TEST_IDS.updateCard` 可见作为同步点（不依赖默认页面内容）。
  - fake updater 版本基线：为避免客户端版本号提升后硬编码远端 `0.0.2` 变成降级，测试内注入 `PARA_APP_VERSION=0.0.1`，让 fake updater 的 semver 比较保持与用例叙事一致。
- 核心变更：仅修改 `client/playwright/electron-update-rollback.spec.ts`（证据路径/头部 + 路由导航 + `PARA_APP_VERSION` 注入）。
- 验证：`npm -C client run e2e -- electron-update-rollback.spec.ts` PASS；证据：`.sisyphus/evidence/task-20-update-rollback.txt`。

## [2026-02-20 20:34] - 任务：修复用户可见 `#/updates`（不再渲染 legacy debug panel）
- 原始需求：修复 `/updates` 路由，避免渲染 `client/src/renderer/app/App.tsx` 的 legacy debug panel，从而杜绝禁用文案（调试/Task/最小 UI/离线模拟）泄露；并保持更新 UI 的 `TEST_IDS` 锚点以确保 `electron-update-rollback` E2E 不回归。
- 实现目标/逻辑：
  - 新增独立页面 `UpdatesPage`，在 `HashRouter` 下将 `/updates` 路由改为渲染用户页面（`AppShell` + 更新面板），不再引用 legacy `App`。
  - Updates 页面复用既有更新交互（check/download/install + onState 订阅），并挂载 `TEST_IDS.updateCard/updateCheck/updateDownload/updateInstall/updateStatus`。
- 核心变更：
  - `client/src/renderer/app/RouterApp.tsx`：`/updates` -> `<UpdatesPage />`。
  - `client/src/renderer/app/pages/UpdatesPage.tsx`：新增更新页面（含 TEST_IDS 锚点与更新状态展示）。
- 遗留/约束：不改主进程更新行为；仅做 renderer 路由/页面修复。
- 验证：`npm -C client run e2e -- electron-update-rollback.spec.ts` -> 1 passed。

## [2026-02-21 12:52] - 任务：Task 4（提交 `.ai_session.md`：split-commits + PR 策略记录）
- 原始需求：在本计划内，仅提交脱敏后的 `.ai_session.md` 记录，补记 split-commits 的约束与 PR 策略；不得写入任何真实凭据/内网地址。
- 实现目标/逻辑：
  - 分支/PR：在新分支开发，PR base=main；禁止直接 push main。
  - 拆分提交：至少 2 个 commit，分别用于“CI gate + evidence”与“session log”，保持可审阅、可回滚的原子性。
  - 提交边界：`.sisyphus/boulder.json` 不提交；evidence 文件最小化，仅用于审计留痕，不与代码混在同一 commit。
  - 最小验证集合：`npm -C client run lint`、`npm -C client test`、`npm -C client run build`；以及 e2e smoke/security 基线（示例：`npm -C client run e2e -- electron-smoke.spec.ts`、`npm -C client run e2e -- electron-a11y-smoke.spec.ts`、`npm -C client run e2e -- electron-security.spec.ts`）。
- 遗留/约束：任何 token/key/password 示例一律使用 `REDACTED`/`<PLACEHOLDER>`；示例 host 仅允许 `127.0.0.1`/`localhost`；不在仓库中保存服务器侧执行证据。

## [2026-02-21 20:29] - 任务：收尾（README 占位补齐 + evidence 入库 + 合并 main）
- 原始需求：继续把剩余工作全部完成，并合并到 main 后推送。
- 实现目标/逻辑：
  - 文档：将 `admin-web/README.md` 与 `server/README.md` 从模板/占位收口为本项目入口说明（指向 runbook/部署文档）。
  - 证据：补齐 split-commits/PR 执行过程证据文件（放入 `.sisyphus/evidence/`，仅包含脱敏输出）。
  - 合并策略：优先走现有 PR（#1）合并到 `main`；如仓库允许直推则本地 fast-forward 后 push `main`（不 force）。
- 核心变更：
  - `admin-web/README.md`
  - `server/README.md`
  ...
  - `.sisyphus/evidence/task-5-min-verify.txt`
  - `.sisyphus/evidence/task-6-pr-created.txt`
  - `.ai_session.md`
- 验证（本地最小门禁）：`./scripts/ci.sh` -> `ALL GREEN`（scan_blockers OK；server pytest 90 passed；client lint/test OK）。
- 合并与推送：PR #1 已合并（mergeCommit=`0bd1d4f`），并已 push 到 `origin/main`。
- 遗留/约束：禁止把任何真实 key/token/内网凭据写入仓库；仅允许 `REDACTED`/占位符。

## [2026-02-21 21:45] - 任务：Server：User 增加 debug_allowed 字段 + Alembic 迁移
- 原始需求：为 User 增加 `debug_allowed` 字段（默认 false）并提供 Alembic 迁移；仅做 schema 最小增量，兼容已有数据。
- 实现目标/逻辑：
  - ORM：`server/app/db/models.py` 在 `User` 增加 `debug_allowed`（`nullable=False`，默认 `False`，并设置 `server_default=false()` 以对齐 DB 默认）。
  - Migration：新增 `server/alembic/versions/7904ac92bb9f_debug_allowed.py`，对 `users` 表新增 `debug_allowed`（`NOT NULL` + `DEFAULT false`），保证旧行不会出现 `NULL`。
- 核心变更：`server/app/db/models.py`、`server/alembic/versions/7904ac92bb9f_debug_allowed.py`。
- 验证：
  - `cd server && uv run alembic upgrade head` -> exit 0
  - `cd server && uv run pytest` -> 90 passed
- 备注：若本地库已存在表但缺失 `alembic_version`，需先 `uv run alembic stamp <已存在的最新 revision>` 再 upgrade，以避免 `DuplicateTable`。

## [2026-02-21 23:40] - 任务：Desktop：para.security.json 扩展 /dev 开关 + IPC（main 端）
- 原始需求：在 Electron main 端扩展 `para.security.json`，新增 /dev 开关（`developerOptionsEnabled`），并提供 IPC `security:devOptions:getStatus` / `security:devOptions:setEnabled`；在生产构建中需基于 `/api/v1/auth/me` 的 `debug_allowed` 做 fail-closed gating。
- 实现目标/逻辑：
  - `para.security.json`：保持 `version: 1` 不变，新增可选字段 `developerOptionsEnabled?: boolean`；读写保持兼容 `appEncEnabled`。
  - 原子写入 merge：写入时先读取旧配置并 merge，避免切换某一个开关时把另一个开关意外覆盖为 `false`。
  - 状态计算：返回 `{ desiredEnabled, effectiveEnabled, error }`；`!app.isPackaged` 时 `effective=desired`；`app.isPackaged` 时 `effective=desired && logged_in && debug_allowed`，网络/未登录/未授权等一律 fail-closed（`effective=false` + 稳定错误码）。
  - 避免多余网络调用：当 `desired=false` 时不请求 `/api/v1/auth/me`。

## [2026-02-21 23:00] - 任务：Electron：SettingsPage 增加“开发者选项（/dev）”开关（renderer UI）
- 原始需求：Settings 页面提供 /dev 开关；首屏读取 status；点击开关写入 desired 并刷新；展示 desired/effective/原因；正式包需登录且后端授权 `debug_allowed` 才会生效；E2E 通过 data-testid 稳定定位。
- 实现目标/逻辑：
  - renderer 通过 `getDesktopApi()?.security.getDevOptionsStatus()` 初始化；开关调用 `setDevOptionsEnabled(next)` 并在返回后刷新状态。
  - reason 展示稳定码（`OK` / `DISABLED` / `NOT_LOGGED_IN` / `DEBUG_NOT_ALLOWED` / `NETWORK_ERROR` / `AUTH_ME_FAILED`），并保持 fail-closed（接口不可用/异常时按关闭处理）。
- 核心变更：
  - `client/src/renderer/app/pages/SettingsPage.tsx`：新增“开发者选项（/dev）”卡片与交互（busy/刷新/错误提示）。
  - `client/src/renderer/app/testIds.ts`：新增 `devOptionsToggle` / `devOptionsEffective` / `devOptionsReason`。
- 遗留/约束：不修改路由守卫；不新增依赖；不改 main/preload IPC。

## [2026-02-21 23:10] - 修复：Settings /dev 首屏状态加载走 refreshDevOptionsStatus
- 问题：首次渲染未统一走 `refreshDevOptionsStatus()`，导致存在场景下首屏 status 未按同一路径加载/归一。
- 修复：`client/src/renderer/app/pages/SettingsPage.tsx` 在首屏 `useEffect` 调用 `refreshDevOptionsStatus({ silent: true })`，并将 `refreshDevOptionsStatus` 用 `useCallback` 固定引用，避免 hook 依赖告警与重复触发。
- 核心变更：`client/src/main/index.ts`。
- 验证：`npm -C client run lint` 通过；对 `client/src/main/index.ts` 运行 `lsp_diagnostics` 无报错。

## [2026-02-21 22:42] - 任务：Desktop：preload 暴露 developerOptions API + renderer typings
- 原始需求：在 Electron preload 暴露 developerOptions 的 get/set API（替代 renderer 直接读 `PARA_DEV_MODE`），并补齐类型声明；返回值必须是安全对象（仅布尔 + error code + configPath）。
- 实现目标/逻辑：
  - Preload：在 `desktopApi.security` 下新增 `getDevOptionsStatus()` / `setDevOptionsEnabled(enabled)`，分别 invoke `security:devOptions:getStatus` / `security:devOptions:setEnabled`。
  - 返回结构：统一为 `{ desiredEnabled, effectiveEnabled, error, configPath }`（不暴露任何 secrets 或配置文件内容）。
  - 兼容：保留 `desktopApi.devModeEnabled`（历史字段），但新代码应优先改用 `desktopApi.security.getDevOptionsStatus()`。
- 核心变更：`client/src/preload/index.ts`、`client/src/renderer/vite-env.d.ts`。
- 验证：`npm -C client run lint` 通过。

## [2026-02-21 23:59] - 任务：Server：挂载 admin_users router，并确保 OpenAPI/合约更新路径可用
- 原始需求：确认 `server/app/api/v1/router.py` 已包含 `admin_users` router（挂载到 `/api/v1`），并通过脚本重新生成 `contracts/openapi.json` 与 `client/src/gen/openapi.ts`；验证 OpenAPI 含 debug_allowed 管理端点。
- 实现目标/逻辑：保持改动最小；不手改生成文件，统一通过 `scripts/generate-contracts.sh` 驱动导出与生成。
- 核心变更：无业务代码改动（`server/app/api/v1/router.py` 已包含 `admin_users_router`）。
- 验证：
  - `./scripts/generate-contracts.sh`
  - `cd server && uv run python -m app.scripts.export_openapi --output ../contracts/openapi.json`
  - `cd client && npm run gen:api`
  - 合约检查：`contracts/openapi.json` 包含 `/api/v1/admin/users/debug_allowed`，operationId 包含 `admin_users_debug_allowed_get|put`。

## [2026-02-22 00:30] - 任务：Task 13（文档与服务器联调固化：AppEnc + /dev 权限在 u.pscly.cc 走通）
- 原始需求：仅改文档并产出脱敏 evidence；说明 Electron “开发者选项（/dev）”开关的 desired/effective 逻辑（packaged 下必须已登录 + `debug_allowed=true` 才生效，fail-closed）；说明 AppEnc（AES-GCM）启用方式与限制（仅 `application/json` 且非空 body；不覆盖 GET/空 body；keys 仅走 env/服务器 `.env`，不得泄密）；在 `u.pscly.cc`（SSH root）确认部署形态并给出可复制粘贴的验证步骤（health + AppEnc fail-closed）。
- 实现目标/逻辑：
  - AppEnc 代码读取：确认服务端 middleware 仅在 `X-Para-Enc: v1` 时启用；对非法 envelope 返回稳定错误码（例如 `PARA_APPENC_BAD_ENVELOPE`），可用于无凭据验证。
  - 远端验证：通过 SSH 在 `u.pscly.cc` 使用 `docker compose ls` 定位运行中的 para 项目，并用 `docker compose -p para -f /root/dockers/para/app/deploy/prod/docker-compose.yml ps` 验证容器形态；用 `curl -fsS https://para.pscly.cc/api/v1/health` 验证反代 health；用 bad-envelope 请求验证 AppEnc fail-closed。
  - 文档：新增 `client/README.md` 解释 /dev desired/effective；在 `docs/runbook.md` 与 `deploy/prod/README.md` 增加 AppEnc 环境变量/范围限制/最小验证与反代下验收步骤。
- 核心变更：`client/README.md`、`docs/runbook.md`、`deploy/prod/README.md`。
- 证据：`.sisyphus/evidence/task-13-health.json`、`.sisyphus/evidence/task-13-appenc-verify.txt`。
- 遗留/约束：严格禁止把任何真实 key/token/密码/内网地址写入仓库或 evidence；示例全部使用 `REDACTED`/`<YOUR_...>` 占位符。

## [2026-02-22 00:39] - 任务：F2（测试与构建门禁：server/client/admin-web）
- 原始需求：按顺序运行 `cd server && uv run pytest`、`cd client && npm run lint && npm test`、`cd admin-web && npm run build`，并报告关键结果/耗时/是否存在泄密风险输出；结果追加写入 notepad。
- 实现目标/逻辑：仅执行门禁命令与最小脱敏扫描；不修改任何业务代码；输出中不包含任何疑似敏感值。
- 核心结果：
  - server：`uv run pytest` exit 0；`94 passed, 1001 warnings`（约 46s）；log：`/tmp/gating-server-pytest-20260222003555.log`。
  - client：`npm run lint` exit 0（约 6s）；log：`/tmp/gating-client-lint-20260222003715.log`。
  - client：`npm test` exit 0；`Test Files 3 passed, Tests 5 passed`（约 3s）；log：`/tmp/gating-client-test-20260222003758.log`。
  - admin-web：`npm run build` exit 0；`built in 1.88s`（约 6s）；log：`/tmp/gating-admin-web-build-20260222003816.log`。
- 泄密风险扫描：未命中 `sk-`/DSN/`Authorization`/JWT 等模式；server pytest log 仅出现 `token` 关键字（词面），未见疑似明文 token 输出。
- 证据与记录：追加到 `.sisyphus/notepads/desktop-debug-gating-appenc/learnings.md` 与 `.sisyphus/notepads/desktop-debug-gating-appenc/issues.md`。

## [2026-02-22 14:55] - 任务：Task 4 后端：admin feature flags 增加 open_registration_enabled（默认 false + 审计）
- 原始需求：admin GET/PUT feature_flags 增加 `open_registration_enabled`；默认值安全（false）；PUT 变更需写入 `AuditLog(action='feature_flags.update')` 且 `changed_keys` 包含该字段；公共 `GET /api/v1/feature_flags` 也需下发该字段。
- 实现目标/逻辑：
  - 默认值：在 `_default_feature_flags()` 增加 `open_registration_enabled=false`，保证 DB 未存 overrides 时仍 fail-closed。
  - PUT 校验：allowed_keys/provided_keys/类型校验均纳入该字段；值等于默认值时从 overrides 中移除，保持 AdminKV 仅存差异。
  - 审计：沿用既有 `feature_flags.update` 审计写法，通过 merged 前后 diff 生成 `changed_keys`。
- 核心变更：
  - `server/app/api/v1/feature_flags.py`：扩展默认 flags 并在 public GET 合并 overrides 时下发该字段。
  - `server/app/api/v1/admin_config.py`：扩展默认 flags；PUT 增加 allowed_keys/provided_keys/类型校验；审计自动覆盖该字段。
  - `server/tests/test_task_22_admin_flags.py`：扩展断言覆盖 public/admin GET 下发该字段，并新增用例验证审计 `changed_keys` 覆盖 `open_registration_enabled`。
- 遗留/约束：本次仅对齐 `admin-web/src/lib/api.ts` 的类型（不涉及 UI 改动）；具体页面展示/交互可按后续任务再接入。

## [2026-02-22 15:20] - 任务：工作区 hygiene（回退非预期 evidence 与测试漂移）
- 原始需求：整理当前工作区的非预期改动（尤其是 `.sisyphus/evidence/**` UUID/时间戳 churn 与测试文件漂移），同时保留 Wave 1 的 username 迁移与 `open_registration_enabled` flag 工作。
- 实现目标/逻辑：仅保留计划验收所需的 6 个文件改动 + 迁移文件，回退所有“注册强制 username/open registration 语义”扩散导致的改动，避免把下一波改动混入当前工作树。
- 核心变更：
  - 回退：`.sisyphus/evidence/**`、`server/app/api/v1/auth.py`、`server/tests/**`（排除 `server/tests/test_task_22_admin_flags.py`）。
  - 保留：`server/app/db/models.py`、`server/alembic/versions/8f35e9f1a5a2_users_username.py`、`server/app/api/v1/admin_config.py`、`server/app/api/v1/feature_flags.py`、`admin-web/src/lib/api.ts`、`server/tests/test_task_22_admin_flags.py`。
- 验证：`git status` 仅剩以上文件（另含 `.ai_session.md`）为变更状态。

## [2026-02-22 15:21] - 任务：Task 2（后端注册：username + open_registration_enabled；仍兼容邀请码路径）
- 原始需求：`POST /api/v1/auth/register` 接受 `username` 并写入 `users.username`（小写化）；非 bootstrap 时按 feature flags 决定是否必须邀请码；保持邀请码校验/消耗逻辑不回归；新增/调整 pytest 覆盖并确保 `cd server && uv run pytest -q` 通过。
- 实现目标/逻辑：
  - username：在 register 层做 `strip().lower()`，并强校验 `^[a-z0-9_]{3,32}$`；缺失/非法返回 400（detail=`username_required`/`username_invalid`）；冲突返回 409（detail=`username_already_exists`）。
  - 注册策略（非 bootstrap）：
    - `invite_registration_enabled=false` -> 403 `registration_closed`（语义/默认值保持不变）；
    - 在其为 true 的前提下，`open_registration_enabled=true` 时允许不带 `invite_code`；否则缺失 `invite_code` 仍 403 `invite_code_required`。
  - 邀请码：仍支持提供 `invite_code` 时的校验/锁定/消耗与 redemption 记录；`open_registration_enabled` 仅影响“缺失邀请码是否允许”。
  - 读取容错：`open_registration_enabled` 从 `AdminKV(feature_flags/global)` 读取，KV/字段缺失或非 bool -> False（fail-closed）。
- 核心变更：
  - `server/app/api/v1/auth.py`：RegisterRequest 增加 `username`；新增 `_open_registration_enabled()`；register() 增加 username 校验/写入 + open registration 分支。
  - `server/tests/*`：为所有 register 调用补齐 username，并新增用例覆盖 `open_registration_enabled=true` 时“缺邀请码仍可注册”。
- 验证：`cd server && uv run pytest -q` -> 95 passed。

## [2026-02-22 15:39] - 任务：Task 2（修正：register username 可选 + 开放注册 flag 语义落地）
- 原始需求：`POST /api/v1/auth/register` 支持 `username`（不破坏旧 client 的 email/password-only 路径）；非 bootstrap 时按 `invite_registration_enabled` + `open_registration_enabled` 组合门控邀请码；保持邀请码校验/消耗与 redemption 记录。
- 实现目标/逻辑：
  - username：RegisterRequest 增加 `username: str | None`；缺失/空白时从 email local-part 生成（lower + 非法字符->`_` + 压缩/去首尾 `_` + 截断到 32，要求 3-32），不足则 fallback `user_<uuid去横杠前缀>`；显式 username 仅 `strip().lower()` 后严格校验 `^[a-z0-9_]{3,32}$`；冲突返回 409 `username_taken`。
  - 注册策略（非 bootstrap）：
    - `invite_registration_enabled=false` -> 403 `registration_closed`（优先级最高，语义不变）；
    - 在其为 true 的前提下，`open_registration_enabled=true` 时允许不带 `invite_code`；open=false 且缺失 `invite_code` -> 403 `invite_code_required`。
    - 兼容性：保持“重复邮箱注册”优先 409 的旧行为，不被 `invite_code_required` 抢先。
- 核心变更：`server/app/api/v1/auth.py`、`server/tests/test_task_2_0_bootstrap_admin_registration.py`。
- 验证：`cd server && uv run pytest -q` -> 98 passed。
- 备注：本条用于更正 15:21 条目中“username 必填/错误码 username_required/username_already_exists”等已过期描述，避免后续任务误读。

## [2026-02-22 16:24] - 任务：Task 6（admin-web：Feature Flags 页面增加“开放注册”开关）
- 原始需求：在 `admin-web` 的 Feature Flags 页面新增第三个开关 `open_registration_enabled`，可读取/切换/保存；保存仍保持 partial update（只提交变更字段）；同步更新 web-admin Playwright stub 并断言“仅切换该字段时 PUT body 只包含该字段”；`npm -C admin-web run build` 需通过。
- 实现目标/逻辑：
  - UI：复用 `FeatureFlagsPage` 既有 `kv/row/switch/badge` 结构；新增本地 state `openRegistrationEnabled`，remote 缺失时默认 `false`；文案强调“开启后允许无邀请码注册”，并明确 `invite_registration_enabled=false` 时仍会关闭注册。
  - 数据一致性：dirty 计算/重置/保存 payload 均纳入该字段；保存时仅对比 remote 与本地差异生成 payload（diff-only）。
  - E2E：stub backend 的 `GET/PUT /api/v1/admin/config/feature_flags` 与 `GET /api/v1/feature_flags` 均包含 `open_registration_enabled`；用例新增断言 `{ open_registration_enabled: true }` 的单字段 PUT body。
- 核心变更：`admin-web/src/pages/FeatureFlagsPage.tsx`、`client/playwright/web-admin-feature-flags.spec.ts`。
- 验证：`npm -C admin-web run build` 通过。

## [2026-02-22 19:03] - 任务：Task 11（客户端：注册/登录页面互链 + 首次引导（最小））
- 原始需求：登录页增加“没有账号？去注册”跳转 `#/register`；注册页增加“已有账号？去登录”跳转 `#/login`；保持改动最小并通过 `npm -C client run lint`。
- 实现目标/逻辑：在两个页面内引入 `react-router-dom` 的 `Link`，使用 `to="/register"` 与 `to="/login"`（HashRouter 下会落到 `#/register` / `#/login`），并用最小 inline 样式保证可见且可聚焦。
- 核心变更：`client/src/renderer/app/pages/LoginPage.tsx`、`client/src/renderer/app/pages/RegisterPage.tsx`。
- 验证：`npm -C client run lint` 通过。
