...
import {
  adminReviewUgcApprove,
  adminReviewUgcDetail,
  adminReviewUgcQueueList,
  adminReviewUgcReject,
  adminReviewUgcSetNote,
  ApiError,
  type AdminReviewUgcDetail,
  type AdminReviewUgcQueueItem,
} from "../lib/api";
...
export function UgcReviewPage() {
  ...
  const [items, setItems] = useState<AdminReviewUgcQueueItem[]>([]);
  const [nextOffset, setNextOffset] = useState<number | null>(null);

  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);
  const [detail, setDetail] = useState<AdminReviewUgcDetail | null>(null);
  ...
  const loadQueue = useCallback(
    async (next: { offset: number }) => {
      ...
      try {
        const res = await adminReviewUgcQueueList({ status, limit, offset: next.offset });
        setItems(res.items);
  ...
  const loadDetail = useCallback(async (assetId: string) => {
    ...
    try {
      const res = await adminReviewUgcDetail(assetId);
      setDetail(res);
  ...
  async function saveNote() {
    ...
    try {
      ...
      const res = await adminReviewUgcSetNote(selectedId, normalized);
      setDetail(res);
      setNoteDraft(res.review_note ?? "");
    } catch (err) {
      if (err instanceof ApiError) setDetailError(err.message);
  ...
  async function act(kind: "approve" | "reject") {
    ...
    setDetailError(null);
    try {
      if (kind === "approve") await adminReviewUgcApprove(selectedId);
      else await adminReviewUgcReject(selectedId);
            ...
            <table className="table">
              ...
              <tbody>
                {items.length === 0 ? (
                  ...
                  items.map((it) => {
                    const isSel = selectedId === it.asset_id;
                    return (
                      <tr
                        key={it.asset_id}
                        style={{ cursor: "pointer", background: isSel ? "rgba(15, 118, 110, 0.06)" : undefined }}
                        onClick={() => {
                          setSelectedId(it.asset_id);
                          void loadDetail(it.asset_id);
                        }}
                      >
                        <td>
        ...
        <section className="card">
          ...
          {!selectedId ? (
            ...
            <div style={{ marginTop: 12 }}>
              <div className="kv">
                <div className="kv-k">asset_id</div>
                <div className="kv-v">
              ...
              <div className="kv">
                ...
                <div className="kv-v mono">{detail.created_at}</div>
              </div>
              <div className="kv">
